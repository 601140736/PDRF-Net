...
class PositionalEncoding(nn.Module):
    def __init__(self, model_dim, dropout, max_len=8400):
...
# vi ir vi=(vi+ir)/2+(vi-ir)/2   ir=(vi+ir)/2+(ir-vi)/2

#-----------------交叉融合模块--------------#
# class fusion(nn.Module):
#     def __init__(self, channel, num_heads=2, num_layers=1, dropout=0.1):
#         super().__init__()
#         self.conv1 = nn.Conv2d(in_channels=2, out_channels=1, stride=1, kernel_size=7, padding=3)
#         self.conv2 = nn.Conv2d(in_channels=2, out_channels=1, stride=1, kernel_size=7, padding=3)
#         self.s = nn.Sigmoid()
#
#     def forward(self, x):
#         vi_feature = x[0]
...
#
#         sub_vi_ir = vi_feature - ir_feature
#         ave1 = torch.mean(sub_vi_ir, dim=1, keepdim=True)
#         max1, _ = torch.max(sub_vi_ir, dim=1, keepdim=True)
#         vi_ir_div = sub_vi_ir * self.s(self.conv1(torch.cat([ave1, max1], dim=1)))
#
#         sub_ir_vi = ir_feature - vi_feature
#         ave2 = torch.mean(sub_ir_vi, dim=1, keepdim=True)
#         max2, _ = torch.max(sub_ir_vi, dim=1, keepdim=True)
#         ir_vi_div = sub_ir_vi * self.s(self.conv2(torch.cat([ave2, max2], dim=1)))
#
#         vi_feature = vi_feature + vi_ir_div
#         ir_feature = ir_feature + ir_vi_div
#
#         return torch.cat([vi_feature, ir_feature],dim=1)
...
class IN(nn.Module):
    ...
    def forward(self, x):
        return x
# ##########################################处理极端环境输入###################################
from .dehaze import DehazeNet, dehaze_feature
from .low_enhance import Low_enhance_net, low_enhance_feature
class IN_poir(nn.Module):
    def __init__(self):
        super(IN_poir, self).__init__()
        self.L_N = Low_enhance_net(in_channels=3)
        self.F_N = DehazeNet(in_channel=3)
    ...
    def forward(self, x):
        ...
        r = self.L_N(vi)
        t, a = self.F_N(vi)
        vi = vi * (1-self.w1) + self.w1* dehaze_feature(vi, t, a)
        vi = vi * (1-self.w2) + self.w2* low_enhance_feature(vi, r)
        return torch.cat([vi, ir],dim=1)
...
class fusion(nn.Module):
    def __init__(self,channel, reduction=16):
        ...
        self.se = SE_Block(channel*2, reduction)
...
class CVCI(nn.Module):
    def __init__(self, in_chans=3, embed_dims=32, img_size=16, num_classes=1000, stem_channel=16, fc_dim=1280,
    ...
    def forward(self, x):
        ...
        #print(x.shape, out_feature.shape)
        return out_feature
...
class SPPF(nn.Module):
    """Spatial Pyramid Pooling - Fast (SPPF) lfayer for YOLOv5 by Glenn Jocher."""
...
class MultiHeadCrossAttention(nn.Module):
    def __init__(self, dim, num_heads=8, attn_ratio=0.5):
        ...
        total_dim = self.key_dim * 2 + self.head_dim  # k + q + v per head
        self.qkv_x = nn.Linear(dim, total_dim * num_heads, bias=False)
        self.qkv_y = nn.Linear(dim, total_dim * num_heads, bias=False)
        self.proj = nn.Linear(dim, dim)
    ...
    def forward(self, x, y):
        ...
        qkv_x = self.qkv_x(x).view(B, N, self.num_heads, -1).permute(0, 2, 1, 3)  # (B, H, N, D_all)
        qkv_y = self.qkv_y(y).view(B, N, self.num_heads, -1).permute(0, 2, 1, 3)  # (B, H, N, D_all)

        q_x, k_x, v_x = torch.split(qkv_x, [self.key_dim, self.key_dim, self.head_dim], dim=-1)
        q_y, k_y, v_y = torch.split(qkv_y, [self.key_dim, self.key_dim, self.head_dim], dim=-1)

        # Cross attention: x attends to y
        attn_xy = (q_x @ k_y.transpose(-2, -1)) * self.scale  # (B, H, N, N)
        attn_xy = attn_xy.softmax(dim=-1)
        out_x = attn_xy @ v_y  # (B, H, N, head_dim)

        # Cross attention: y attends to x
        attn_yx = (q_y @ k_x.transpose(-2, -1)) * self.scale
        attn_yx = attn_yx.softmax(dim=-1)
        out_y = attn_yx @ v_x

        # concat heads
        out_x = out_x.transpose(1, 2).contiguous().view(B, N, C)  # (B, N, C)
        out_y = out_y.transpose(1, 2).contiguous().view(B, N, C)

        out_x = self.proj(out_x).permute(0, 2, 1)  # (B, C, N)
        out_y = self.proj(out_y).permute(0, 2, 1)

        return out_x + x.permute(0, 2, 1), out_y + y.permute(0, 2, 1)  # 残差连接
...
class concat_vi(nn.Module):
    def __init__(self):
        super(concat_vi, self).__init__()
...
class MFusion(nn.Module):
    def __init__(self,dim_in):
        ...
        dim = dim_in//4
        self.dim = dim
        self.num_experts = 4 # 这个
        self.top_k = 1

        # 定义多个专家，每个专家是 (B, C, W, H) -> (B, C, W, H)
        self.experts = nn.ModuleList([
            CVCI(in_chans=dim, embed_dims=dim), # 这个
            #FeatureAdd(q=1),
            fusion(channel=dim),
            GPT(d_model=dim),
            #FeatureAdd(q=1),
            #CrossTransformerFusion(input_dim=dim),
            #FeatureAdd(q=1),
            concat(),
        ])
...
class MFusion_1(nn.Module):
    def __init__(self,dim_in):
        super(MFusion_1, self).__init__()
        dim = dim_in//4
        self.dim = dim
        self.num_experts = 4 # 这个
        self.top_k = 1
        ...
        # 定义多个专家，每个专家是 (B, C, W, H) -> (B, C, W, H)
        self.fusion = CVCI(in_chans=dim, embed_dims=dim)


    def forward(self, x):
        return self.fusion(x)

class MFusion_2(nn.Module):
    def __init__(self,dim_in):
        super(MFusion_2, self).__init__()
        dim = dim_in//4
        self.dim = dim
        self.num_experts = 4 # 这个
        self.top_k = 1
        ...
        # 定义多个专家，每个专家是 (B, C, W, H) -> (B, C, W, H)
        self.fusion = fusion(channel=dim)


    def forward(self, x):
        return self.fusion(x)

class MFusion_3(nn.Module):
    def __init__(self,dim_in):
        super(MFusion_3, self).__init__()
        dim = dim_in//4
        self.dim = dim
        self.num_experts = 4 # 这个
        self.top_k = 1
